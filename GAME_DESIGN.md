...
Heian-kyō, ~1005 CE. The Fujiwara regency is at its zenith. You are the child of a provincial governor (zuryō) — newly arrived in the capital with money, a modest residence, and zero pedigree. Over a single court calendar year, you attend festivals, banquets, and contests; solve other people's problems; connect people who need connecting; and decide who you'll love and what you'll trade for rank.
...
- Background fixed for v0.1 (zuryō heir) — it cleanly motivates both halves of the game: tycoon (you have wealth to deploy) and dating sim (you lack standing, so relationships ARE your ladder).
...
### Clout (the year's reputation — resets every New Year)
The one stat that does *not* carry over. Clout is annual notoriety: how much you are *this year's* person.

- **Earned by:** public wins — utaawase victories, well-received poems, hosting, festival moments, visible problem-solving. Public, not private: a favor done quietly earns Favor; a favor done where everyone can see earns Clout.
- **Benefits (scale through the year):** invitation tier (high-Clout players get pulled into events above their Rank gate), Favor-gain multiplier, situational Charisma/Allure bonuses ("everyone wants to be seen with you"). By the 11th month, peak Clout means you're the season's name.
- **The target on your back:** above thresholds, envy events spawn — rivals scheme against you specifically, gossip about you spreads faster and stings harder, you become the subject of other people's ripples. High Clout is high variance by design.
- **Year-end conversion:** at the jimoku (New Year promotions), Clout + faction standing convert into permanent Rank and rewards. Then Clout resets to zero. The annual loop: build it all year, cash it at the promotions list, start the new year as last year's news.

### Multi-year / NG+ (architecture now, content later)
The campaign continues year over year. **Carries over:** attributes, Rank, relationships and romance states, NPC attitudes, faction reputation, household (staff, upgrades, wardrobe), Koku. **Resets annually:** Clout. Save schema must support this from M0 — year index, per-year Clout ledger, persistent everything else. v0.1 ships year 1 only; year-2+ content (higher-rank events, marriage consequences, rival arcs maturing) is post-v0.1, but nothing in the engine may assume a single year.
...
type Choice = { text: string; check?: Check; effects: Effect[]; goto: NodeId };
...
type Save = {
  ...
  clout: number;                                           // resets at New Year
  cloutHistory: Record<number, number>;                    // per-year ledger (jimoku math, flavor)
...
**M0 — Skeleton.** Vite/React/TS scaffold, Zustand stores, save/load to localStorage **with the multi-year save shape (year index, clout ledger) from the start**, calendar tick, navigation shell (household ⇄ event), kasane palette tokens wired to month. Placeholder art throughout (locked decision) — placeholders must respect palette tokens and layout. *Done when: a year can be clicked through with state persisting across reload, and ticking past month 12 rolls to year 2 with Clout zeroed and everything else intact.*
...
**M2 — Household layer + Clout.** Income tick, free actions, staff hiring, attribute training, Composure economy, wardrobe equip with seasonal bonus/penalty, Clout accrual from public effects with benefit tiers and at least one envy-event trigger. *Done when: a player can grind Rhetoric, go broke hiring staff, get gossiped about for wearing autumn colors in spring, and attract a rival's attention by winning too publicly.*
...
**M5 — Raking + gossip surfacing.** Raking mini game with Composure restoration; faction reputation surfacing lines in scenes; events for months 4, 7, 8 content-complete. *Done when: the Aoi Matsuri carriage dispute plays with all options viable and all options costly.*

**M6 — Finale + polish.** Months 11→jimoku ending logic (Clout + faction standing + romance state + rank → 4–6 ending slates, then Clout reset into year 2), **Gloss and Immersion poem modes** (UI work over existing content fields), audio pass (koto/shō ambience), transitions, PWA manifest. *Done when: a full year is completable with at least 3 meaningfully different endings reachable, and year 2 starts cleanly with carried-over state.*
...
- 1 year of *content*, multi-year *architecture*: year 2 must start cleanly (carryover + Clout reset) even though it has no bespoke events yet
...
2. **Multi-year NG+:** yes. Attributes, Rank, relationships, attitudes, reputation, household, and Koku all persist; **Clout** resets each New Year, converting (with faction standing) into permanent Rank at the jimoku first — confirmed. Architecture supports this from M0.
